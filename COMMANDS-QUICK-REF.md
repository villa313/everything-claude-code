...
> 97 slash commands installed globally. Type `/` in any Claude Code session to invoke.
...
| `/csharp-test` | TDD for C# (xUnit first, 80%+ coverage via coverlet / `dotnet test --collect`) |
...
| `/csharp-review` | C# / ASP.NET Core — async correctness, EF Core patterns, nullable reference types, security, idiomatic .NET |
...
| `/csharp-build` | Fix C# / .NET build errors, CS compiler errors, and NuGet dependency issues |
